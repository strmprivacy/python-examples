...
run-sender:
	...
	python3 -u examples/sender.py --billing-id $${billingId} --client-id $${clientId} --client-secret $${clientSecret}

run-synchronous-sender:
	@read -p "Enter [billing id] [client id] [client secret] (separated by spaces):" billingId clientId clientSecret; \
	python3 -u examples/syncsender.py --billing-id $${billingId} --client-id $${clientId} --client-secret $${clientSecret}
